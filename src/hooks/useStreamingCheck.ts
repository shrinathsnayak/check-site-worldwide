...
export function useStreamingCheck() {
  ...
  const startStreaming = useCallback((url: string, countries?: string[]) => {
        ...
        switch (streamingEvent.type) {
          case 'cached': {
            ...
            setState(prev => ({
              ...
              countries: cachedData.countries.map((country: any) => {
                const result = cachedData.results.find(
                  (r: CheckResult) => r.country === country.code
                );
                return {
                  ...country,
          ...
          case 'result': {
            ...
            setState(prev => {
              ...
              console.log(
                `📊 Received result ${newCompletedCount}/${prev.totalCountries} for ${result.country}`
              );
          ...
          case 'complete': {
            ...
            console.log(
              `✅ Stream completed: ${completeData.totalProcessed}/${completeData.totalExpected} countries processed`
            );
            ...
            setState(prev => {
              ...
              if (expectedCount > 0 && actualCount < expectedCount) {
                console.warn(
                  `⚠️ Stream marked complete but missing results: ${actualCount}/${expectedCount}`
                );
                console.warn(
                  `Missing countries:`,
                  prev.countries
              ...
              console.log(
                `✅ All ${actualCount} results received, marking as complete`
              );
              return {
                ...prev,
                isStreaming: false,
  ...
  return {
    ...
    progress:
      state.totalCountries > 0
        ? state.completedCountries / state.totalCountries
        : 0,
  };
}
